#!/usr/bin/env bash
logger -i -t $0 -p user.info "Starting.." &>> /root/AutoAdmin/$(date +%d-%m-%y)-log.txt
#System Parametars
...
do
	...
	then
		...
		logger -s -i -t $0 -p user.info "Alert was sent to $Mail, the resource status right now are CPU=$cpuUsage%,MEM=$memUsage%" &>> /root/AutoAdmin/$(date +%d-%m-%y)-log.txt
		sleep 600
	fi
